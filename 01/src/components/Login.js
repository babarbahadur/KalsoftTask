...
export default class Login extends Component {
  ...
  renderLoginButtonFunction() {
    if (this.state.login_button_click === false) {
      ...
      return (
        <View>
          <Spinner size="large" color={THEME_COLOR} />
        </View>
      );
    }
  }
    ...
    fetch("http://wownow.pk/react-native-rider-app-api/login.php", {
      ...
      .then(responseJson => {
        ...
        if (responseJson.msg === "Success") {
          ...
          AsyncStorage.setItem("session_login_id", responseJson.id);
          this.getDeviceToken(responseJson.id, responseJson.email);
        } else {
          this.setState({ error_msg: true });
  ...
  getDeviceToken = (rider_id, vendor_email) => {
    firebase
      .messaging()
      .getToken()
      .then(token => {
